 ...
 */
class TestActorRefFactory(implicit val anonymousSchedule: TestSchedule, implicit val traceMonitorActor: ActorRef) {

  implicit val testFactory = this
  /**
   ...
   * </pre>
   */
  def actorOf(factory: ⇒ Actor): TestActorRef = new TestActorRef(() ⇒ factory, None)
