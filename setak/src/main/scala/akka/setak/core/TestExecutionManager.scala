 ...
 */
class TestExecutionManager(implicit traceMonitorActor: ActorRef) {
  ...
  def checkForStability(maxTry: Int = TestConfig.maxTryForStability): Boolean =
    {
      var triedCheck = 0
      var isStable = false
      var notProcessedMessages = new ArrayBuffer[RealEnvelop]()
      var reason = ""
        ...
        if (isStable) {
          if (!(traceMonitorActor ? AllDeliveredMessagesAreProcessed).mapTo[Boolean].get) {
            ...
            if (triedCheck == maxTry) {
              ...
              notProcessedMessages = (traceMonitorActor ? NotProcessedMessages).mapTo[ArrayBuffer[RealEnvelop]].get
            }
   ...
   */
  def waitForMessage(messages: Set[TestEnvelop]): Boolean =
    {
      try {
